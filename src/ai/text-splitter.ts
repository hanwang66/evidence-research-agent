  ...
  implements RecursiveCharacterTextSplitterParams
{
  ...
  splitText(text: string): string[] {
    ...
    // With no natural separator, fixed-size chunks avoid creating artificial
    // overlap between every character and keep long identifiers/data intact.
    if (separator === '') {
      const chunks: string[] = [];
      for (let start = 0; start < text.length; start += this.chunkSize) {
